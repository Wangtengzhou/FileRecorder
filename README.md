...
> ⚠️ **内容审查**：某些敏感内容可能触发 AI 服务商的内容审查，导致部分文件无法识别。这些文件会自动进入二次检测流程。

> ⚠️ **自定义标签**：如需自定义标签（如"动漫"改为"日漫"），在 AI 整理弹窗中管理，或直接修改 `config.json` 的 `media_types` 数组。

> ⚠️ **系统预设**：如果需要固定的矫正规则，请在`config.json`中填写 `system_preset` 字段，无需每次运行时输入。
...
- **AI 整理**: AI 整理功能需要配置 AI API 密钥，建议使用Grok来处理NSFW内容。
