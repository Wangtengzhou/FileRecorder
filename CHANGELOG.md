...
- **网络轮询间隔未持久化**：修复用户修改网络监控轮询间隔后，间隔值未正确保存到各目录，重启后恢复默认15分钟的问题
...
- **目录树优化**
  - 左侧目录树添加文件夹图标，与右侧保持一致
  - 只有当目录下有子文件夹时才显示展开箭头
- **删除索引功能**
  - 右侧区域：文件和目录均支持右键删除、Delete 键批量删除
  - 左侧目录树：Delete 键删除选中目录
  - 删除前均有确认弹窗，删除后显示结果弹窗
  - 所有删除操作均记录日志（用于问题排查）
- **监控状态判断优化**：修正目录监控状态判定逻辑，现在只有在目录监控功能启用且具体目录已勾选时才视为监控状态
- **监控与删除功能协同**
  - 添加监控时统一强制扫描：用户必须选择扫描或取消（取消则移除监控）
  - 右键/Delete 删除监控目录下的文件时，显示监控保护弹窗（去除监控/去除并删除/取消）
  - Delete 键批量删除混合选中时，增加「仅删除非监控文件」选项
- **启动对账弹窗优化**
  - 优化弹窗文案，明确说明目录监控功能和不同步后果
  - 禁用弹窗右上角关闭按钮，防止绕过防护
  - 添加启动对账进度弹窗，显示正在检测的目录和进度
  - 对账完成后再启动运行时监控，避免轮询提前执行

### 修复
- **对账检测误报**：修复对账时将目录记录误判为"删除"的问题，现在只检测文件变化
- **轮询绕过对账**：移除网络目录轮询启动时的首次立即检查，由对账器统一处理
- **对账检测过滤规则统一**：对账检测现在使用与扫描器完全相同的配置（`scanner.ignore_patterns`），可在设置中统一管理
---
